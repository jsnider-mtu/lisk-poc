...
const DB_KEY_TRANSACTIONS = "socmed:transactions";
// const CREATENFT_ASSET_ID = 0;
// const PURCHASENFT_ASSET_ID = 1;
// const TRANSFERNFT_ASSET_ID = 2;

// Schemas
const encodedTransactionSchema = {
  $id: 'socmed/encoded/transactions',
  type: 'object',
...
const getDBInstance = async (dataPath = '~/.lisk/socmed-app/', dbName = 'socmed_plugin.db') => {
...
//                 nftHistory = [{ address: Buffer.from(base32Address, 'binary'), salePrice: BigInt(trx.asset.initValue) }, ...savedHistory];
...
//           };
//         });
//       } else if (trx.assetID === TRANSFERNFT_ASSET_ID) {
//         dbKey = `nft:${trx.asset.nftId}`;
//         base32Address = cryptography.getBase32AddressFromAddress(Buffer.from(trx.asset.recipient, 'hex'));
//         savedHistory = await getNFTHistory(db, dbKey);
//         nftHistory = [{ address: Buffer.from(base32Address, 'binary'), salePrice: BigInt(0) }, ...savedHistory];
//         encodedNFTHistory = codec.encode(encodedNFTHistorySchema, { nftHistory });
//         await db.put(dbKey, encodedNFTHistory);
//       } else if (trx.assetID === PURCHASENFT_ASSET_ID) {
//         dbKey = `nft:${trx.asset.nftId}`;
//         base32Address = cryptography.getBase32AddressFromPublicKey(Buffer.from(trx.senderPublicKey, 'hex'), 'lsk');
//         savedHistory = await getNFTHistory(db, dbKey);
//         nftHistory = [{ address: Buffer.from(base32Address, 'binary'), salePrice: BigInt(trx.asset.purchaseValue) }, ...savedHistory];
//         encodedNFTHistory = codec.encode(encodedNFTHistorySchema, { nftHistory });
//         await db.put(dbKey, encodedNFTHistory);
//       }
//     }
